...
export const selectIngredientsByRecipeID = (recipeID) => {
	return new Promise((resolve, reject) => {
		pool.query(
			'SELECT i.Name, i.RecipeID, i.ItemID, i.IngredientID, i.IsMissingUnits, it.name as TagName, it.IngredientTagID, id.Name as IngredientDepartment, id.Position as IngredientDepartmentPosition FROM INGREDIENT i LEFT JOIN INGREDIENT_TAG it ON i.IngredientTagID = it.IngredientTagID LEFT JOIN INGREDIENT_DEPARTMENT id on it.IngredientDepartmentID = id.IngredientDepartmentID WHERE RecipeID = ? ORDER BY i.IngredientID ASC',
			[recipeID],
			(error, result) => {
				if (error) {
					return reject(error.sqlMessage);
...
export const selectIngredientByIngredientID = (ingredientID) => {
	return new Promise((resolve, reject) => {
		pool.query(
