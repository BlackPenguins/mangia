...
export const selectAllShoppingListItem = (weekID) => {
	return new Promise((resolve, reject) => {
		pool.query(
			'SELECT s.IngredientTagID, s.ShoppingListItemID, s.Amount, s.IsChecked, s.RecipeCount, s.RecipeNames, s.IsMissingUnits, t.Name as TagName, d.Name as Department, d.Position as DepartmentPosition FROM SHOPPING_LIST_ITEM s LEFT JOIN INGREDIENT_TAG t ON s.IngredientTagID = t.IngredientTagID LEFT JOIN INGREDIENT_DEPARTMENT d on t.IngredientDepartmentID = d.IngredientDepartmentID WHERE WeekID = ?',
			[weekID],
			(error, result) => {
				if (error) {
