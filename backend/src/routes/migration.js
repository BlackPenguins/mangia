		...
		switch (migrationKeyword) {
					...
					for (const recipe of recipes) {
						if (recipe.Image == null) {
							...
							if (fs.existsSync(`./images/thumbs/${beforeImageFileName}`)) {
								...
								await resizeThumbnail(recipe.RecipeID, './images/thumbs', beforeImageFileName, afterImageFileName, true);
							} else {
			...
			case 'moveThumbnails':
				...
				for (const recipe of allRecipes) {
					...
					if (image) {
						await insertThumbnail(id, image, false);
